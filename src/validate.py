...
def validate(weights_path: str = "weights/best_crypto_model.pth"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    dataset = CryptoMultimodalDataset(
        binance_csv="data/processed/binance_btc_1h_features.csv",
        sentiment_csv="data/processed/crypto_sentiment_1h.csv",
        seq_length=60,
        is_training=False,
    )
    ...
    _print_metrics("Model", preds, targets)
...
def _print_metrics(name: str, preds: np.ndarray, targets: np.ndarray):
    ...
    corr     = np.corrcoef(preds, targets)[0, 1] if preds.std() > 0 else 0.0
